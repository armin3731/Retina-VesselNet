...
def get_dataset(dataset_cfg, regenerate=True):
    ## load configs
    dataset_path = kagglehub.dataset_download(dataset_cfg["dataset_path"])
    # modify the dataset_path to your own dir（将dataset_path修改至你自己的路径）
    train_dir = os.path.join(dataset_path, dataset_cfg["train_dir"])
    train_image_dir = os.path.join(train_dir, dataset_cfg["train_image_dir"])
    train_mask_dir = os.path.join(train_dir, dataset_cfg["train_mask_dir"])
    train_groundtruth_dir = os.path.join(
        train_dir, dataset_cfg["train_groundtruth_dir"]
    )
    train_patch_dir = os.path.join(train_dir, dataset_cfg["train_patch_dir"])

    # scan the training image
    train_image_path_list = glob(os.path.join(train_image_dir, "*.tif"))

    # split training/val set
    val_image_path_list = random.sample(
        train_image_path_list, int(len(train_image_path_list) * (1 - train_val_rate))
    )
    train_image_path_list = [
        i for i in train_image_path_list if i not in val_image_path_list
    ]
    ...
    if regenerate:
        ...
        for i in tqdm(
            range(len(train_image_path_list)), desc="Generate the training patches: "
        ):
            image2patch(
                train_image_path_list[i],
                patch_num,
                patch_size,
                patch_threshold,
                train_patch_dir,
                ...
                training=True,
                show=False,
            )  # set show=True to visualize the sample process, which is much slower than show=False

        for i in tqdm(
            range(len(val_image_path_list)), desc="Generate the val patches: "
        ):
            image2patch(
                val_image_path_list[i],
                patch_num,
                patch_size,
                patch_threshold,
                train_patch_dir,
    ...
    train_patch_img_path_list = sorted(
        glob(os.path.join(train_patch_dir, "*-*-img.jpg"))
    )
    train_patch_groundtruth_path_list = sorted(
        glob(os.path.join(train_patch_dir, "*-*-groundtruth.jpg"))
    )
    train_patch_img_path_list, train_patch_groundtruth_path_list = shuffle(
        train_patch_img_path_list, train_patch_groundtruth_path_list, random_state=0
    )
    ...
    val_patch_img_path_list = sorted(
        glob(os.path.join(train_patch_dir, "*_*_val_img.jpg"))
    )
    val_patch_groundtruth_path_list = sorted(
        glob(os.path.join(train_patch_dir, "*_*_val_groundtruth.jpg"))
    )
    ...
    train_dataset = tf.data.Dataset.from_tensor_slices(
        (train_patch_img_path_list, train_patch_groundtruth_path_list)
    )
    train_dataset = train_dataset.map(
        load_image_groundtruth, num_parallel_calls=tf.data.experimental.AUTOTUNE
    )
    # train_dataset = train_dataset.shuffle(buffer_size=1300).prefetch(BATCH_SIZE).batch(BATCH_SIZE)

    # VAL Dataloader
    val_dataset = tf.data.Dataset.from_tensor_slices(
        (val_patch_img_path_list, val_patch_groundtruth_path_list)
    )
