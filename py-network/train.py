    ...
    with log_writer.as_default():
        for epoch in range(EPOCHS):
            ...
            for tstep, (patch, groundtruth) in enumerate(train_dataset):
                ...
                tf.summary.scalar(
                    "learning_rate", optimizer.learning_rate.numpy(), step=lr_step
                )
                print(
                    "\repoch {}, batch {}, loss:{:.4f}, dice:{:.4f}".format(
                        epoch + 1, tstep, train_loss.result(), train_acc.result()
                    ),
                    end="",
                )
                lr_step += 1
            ...
            if (epoch + 1) % VAL_TIME == 0:
                ...
                print(
                    "\repoch {}, batch {}, train_loss:{:.4f}, train_dice:{:.4f}, val_loss:{:.4f}, val_dice:{:.4f}".format(
                        epoch + 1,
                        vstep,
                        train_loss.result(),
                        train_acc.result(),
...
if __name__ == "__main__":
    ...
    train_dataset = (
        train_dataset.shuffle(buffer_size=1300)
        .prefetch(train_cfg["batch_size"])
        .batch(train_cfg["batch_size"])
    )
    val_dataset = (
        val_dataset.shuffle(buffer_size=1300)
        .prefetch(train_cfg["batch_size"])
        .batch(train_cfg["batch_size"])
    )
