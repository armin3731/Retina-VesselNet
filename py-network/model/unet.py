...
class LinearTransform(tf.keras.Model):
    def __init__(self, patch_size, name="LinearTransform"):
        ...
        self.conv_r = Conv2D(
            1, kernel_size=3, strides=1, padding="same", use_bias=False
        )
        self.conv_g = Conv2D(
            1, kernel_size=3, strides=1, padding="same", use_bias=False
        )
    ...
    def call(self, input, training=True):
        ...
        output = self.bn(
            merge[:, :, :, 0:1] * r + merge[:, :, :, 1:2] * g + merge[:, :, :, 2:3] * b,
            training=training,
        )
        return output
    ...
    def __init__(self, out_ch, residual_path=False, stride=1):
        ...
        if residual_path:
            self.conv_shortcut = Conv2D(
                out_ch, kernel_size=1, strides=stride, padding="same", use_bias=False
            )
...
class Unet(tf.keras.Model):
    def __init__(self, patch_size):
        ...
        self.conv_final = Conv2D(
            1, kernel_size=1, strides=1, padding="same", use_bias=False
        )
